...
export class Snippet {
   ...
   *
   * Two modes:
   *   - Section mode (default, length = null): returns the full section content
   *     rendered as markdown (heading + content blocks), or the full body when
   *     the document has no structure.
   *   - Range mode (length specified): always extracts a substring of the raw
   *     body around the match position, ignoring section boundaries.
   *     start = max(0, position + offset), end = start + length.
   *
   * @param {object} options
   * @param {number|null} options.length  - Characters to extract (null = full section/body)
   * @param {number}      options.offset  - Offset from match position, used only in range mode (default: 0)
   * @returns {string} Text
   */
  getText({ length = null, offset = 0 } = {}) {
    // --- Range mode: length explicitly provided ---
    if (length !== null) {
    ...
    // --- Section mode: return full section content ---
    if (this.hasContent()) {
    ...
    // --- Section mode, no structure: return full body ---
