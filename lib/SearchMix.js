...
import { epubToMarkdown } from "./epub-converter.js";
import { pdfToMarkdown } from "./pdf-converter.js";
import { srtToMarkdown } from "./srt-converter.js";
import { txtToMarkdown } from "./txt-converter.js";
...
export class SearchMix {
   ...
   * @param {boolean} options.allOccurrences - Return all occurrences as flat list (default: true)
   * @param {number} options.maxOccurrences - Maximum occurrences per document (default: 10)
   * @returns {object} Search results: { results: [Snippet, ...], totalCount: n, totalSnippets: m }
   */
  search(query, {
    limit = 20,
    minScore = null,
    ...
    for (const result of results) {
      ...
      if (snippets) {
        ...
        if (allOccurrences) {
          const rawSnippets = this._extractAllSnippets(query, result.title, result.h1, result.h2, result.h3, result.h4, result.h5, result.h6, result.body, snippetLength, maxOccurrences, structure, sectionsIndex, result.path);
          ...
          const rawSnippet = this._extractSnippet(query, result.title, result.h1, result.h2, result.h3, result.h4, result.h5, result.h6, result.body, snippetLength, structure, sectionsIndex, result.path);
   ...
   * @private
   */
  _extractAllSnippets(query, title, h1, h2, h3, h4, h5, h6, body, maxLength, maxOccurrences, structure = [], sectionsIndex = {}, documentPath = '') {
    ...
    const searchTerms = query
      ...
      .map(term => this._normalizeText(term));
    ...
    for (const section of sections) {
      ...
      if (section.type.startsWith('h')) {
        ...
        for (const term of searchTerms) {
          for (let i = 0; i < normalizedHeadingLines.length && snippets.length < maxOccurrences; i++) {
            if (normalizedHeadingLines[i].includes(term)) {
          ...
          if (snippets.length >= maxOccurrences) break;
        }
      } else {
        ...
        for (const term of searchTerms) {
          ...
          while (startIndex < normalizedText.length && snippets.length < maxOccurrences) {
  ...
  /**
   * Extract snippet showing context around search terms
   * @private
   */
  _extractSnippet(query, title, h1, h2, h3, h4, h5, h6, body, maxLength, structure = [], sectionsIndex = {}, documentPath = '') {
    // Clean query - remove FTS5 operators and get search terms
    const searchTerms = query
      .replace(/title_normalized:|h1_normalized:|h2_normalized:|h3_normalized:|h4_normalized:|h5_normalized:|h6_normalized:|body_normalized:|title:|h1:|h2:|h3:|h4:|h5:|h6:|headings:|body:/gi, '')
      .replace(/AND|OR|NOT/gi, '')
    ...
    for (const section of sections) {
      ...
      for (const term of searchTerms) {
        ...
        if (index !== -1) {
          ...
          // Try to find corresponding section in structure
          let structureSection = section.type;
          if (Object.keys(sectionsIndex).length > 0) {
            if (section.type.startsWith('h')) {
              structureSection = this._findSectionInStructure(structure, snippet, section.type, sectionsIndex) || section.type;
              ...
              structureSection = findContentSection(structure) || section.type;
            }
          }

          return this._createNavigableSnippet(snippet, structureSection, index, structure, sectionsIndex, documentPath);
        }
      }
    }

    // Fallback: return start of body or title
    const fallbackText = body || title || '';
    const fallbackSnippet = fallbackText.length > maxLength
      ? fallbackText.slice(0, maxLength).trim() + '...'
      : fallbackText;
    
    const fallbackSection = body ? 'body' : 'title';
    
    // Try to find first section in structure for fallback
    let structureSection = fallbackSection;
    if (structure.length > 0 && structure[0]) {
      structureSection = structure[0];
    }
    
    return this._createNavigableSnippet(fallbackSnippet, structureSection, 0, structure, sectionsIndex, documentPath);
  }
