...
function readFileWithEncoding(filePath) {
  ...
  // Para subtítulos en español, priorizar encodings comunes
  // Si la confianza es baja o detecta windows-1251/1252, probar con latin1
  const commonEncodings = ["ISO-8859-1", "windows-1252", "UTF-8"];
  
  if (detected.confidence < 0.9 || encoding.toLowerCase().includes("windows-125")) {
    // Probar con varios encodings comunes y verificar caracteres españoles
    for (const testEncoding of commonEncodings) {
      try {
        const testContent = iconv.decode(buffer, testEncoding);
        // Verificar si tiene caracteres españoles válidos
        const spanishChars = /[áéíóúñÁÉÍÓÚÑ¿¡]/;
        if (spanishChars.test(testContent)) {
          encoding = testEncoding;
          console.log(`  Encoding detectado: ${encoding} (auto-corregido)`);
          return testContent;
        }
      } catch (e) {
        continue;
  ...
  console.log(`  Encoding detectado: ${encoding} (confianza: ${(detected.confidence * 100).toFixed(1)}%)`);
...
export async function srtToMarkdown(srtPath, options = {}) {
  try {
    ...
    const sceneGap = options.sceneGap || 5;
